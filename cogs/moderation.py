...
class Moderation(commands.Cog):
    ...
    def save_case_ids(self):
        with open(CASE_FILE_PATH, "w") as file:
            ...
            json.dump({case_id: {"user_id": int(data[0]), "reason": data[1]} for case_id, data in self.case_ids.items()}, file, indent=4)
    ...
    async def timeout(self, ctx, member: discord.Member, duration: int):
        ...
        # Assuming you have a role named "Timeout" set up in your server
        timeout_role = discord.utils.get(ctx.guild.roles, name="Timeout")

        if timeout_role is None:
            await ctx.send("Timeout role not found. Please create a role named 'Timeout'.")
            return
        ...
        # After the timeout duration, remove the timeout role
    ...
    async def un_timeout(self, ctx, member: discord.Member):
        # Check if the user has the necessary permissions to execute this command
        if ctx.author.guild_permissions.ban_members:
            # Attempt to unban the member
            try:
                await ctx.guild.unban(member, reason="Un-timeout")
                await ctx.send(f"{member.mention}'s timeout has been lifted.")
    ...
    async def warn(self, ctx, member: discord.Member, *, reason=None):
        if ctx.author.guild_permissions.manage_roles:
            ...
            self.case_ids[case_id] = (member.id, reason)
            self.save_case_ids()
